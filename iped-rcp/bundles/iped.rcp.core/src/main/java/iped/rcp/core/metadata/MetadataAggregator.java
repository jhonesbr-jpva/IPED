...
/**
 * Facet aggregation over docvalues (task T030, FR-010/MD-01..03): faithful
 * port of the legacy {@code iped.app.metadata.MetadataSearch}/
 * {@code MetadataSearchable} (which live in the Swing UI module and depend
 * on the {@code App} singleton), decoupled to take the case source and the
 * result as inputs. The binning/counting logic is intentionally identical —
 * any change here is a parity break (SC-001).
 *
 * <p>
 * Stateful like the original: {@link #countValues} keeps the linear-scale
 * bin bounds used later by {@link #getIdsWithOrd} to resolve range ords, so
 * the same instance must be used for counting and filtering.
 */
public class MetadataAggregator {

    public static final String EVENT_SEPARATOR = Pattern.quote(IndexItem.EVENT_SEPARATOR);
    public static final String MONEY_FIELD = RegexTask.REGEX_PREFIX + "MONEY";
    ...
    private volatile NumericDocValues numValues;
    private volatile SortedNumericDocValues numValuesSet;
    private volatile SortedDocValues docValues;
    private volatile SortedSetDocValues docValuesSet;
    private volatile SortedSetDocValues eventDocValuesSet;
    private final HashMap<String, long[]> eventSetToOrdsCache = new HashMap<>();

    private volatile boolean logScale = false;
    private volatile boolean noRanges = false;
    private volatile boolean isCategory = false;

    private volatile double min, max, interval;
    private volatile long[] actualMin, actualMax;

    public MetadataAggregator(IPEDMultiSource source) {
        this.source = source;
    }

    /** Localized range separator label (legacy {@code MetadataPanel.RangeSeparator}). */
    static String rangeSeparator() {
        return Messages.getString("MetadataPanel.RangeSeparator");
    }
    ...
    public void setNoRanges(boolean noRanges) {
    ...
    /** Single-valued field probe, after the last load (legacy surface). */
    ...
    /**
     * Filters the result keeping items whose field value falls in the given
     * ords (port of {@code MetadataSearch.getIdsWithOrd}; range ords resolve
     * through the bin bounds computed by the last {@link #countValues} run).
     */
    public MultiSearchResult getIdsWithOrd(MultiSearchResult result, String field, Set<Integer> ordsToGet)
        ...
        if (isNumeric && numValues != null && !noRanges) {
            for (IItemId item : result.getIterator()) {
                int doc = source.getLuceneId(item);
            ...
            ArrayList<Double> l = new ArrayList<>(set);
            Collections.sort(l);
            set.clear();
        ...
        for (int i = 0; i < primitiveScores.length; i++) {
            primitiveScores[i] = scores.get(i);
        }
        return new MultiSearchResult(items.toArray(new IItemId[0]), primitiveScores);
    }
    ...
    private long[] getEventOrdsFromEventSet(SortedSetDocValues eventDocValues, String eventSet) throws IOException {
        ...
        for (int i = 0; i < ords.length; i++) {
            ords[i] = eventDocValues.lookupTerm(new BytesRef(events[i]));
        }
    ...
    /**
     * Counts the values of a field over the given result (port of
     * {@code MetadataSearch.countValues}).
     */
    public ArrayList<ValueCount> countValues(String field, IMultiSearchResult ipedResult) throws IOException {
        field = LocalizedProperties.getNonLocalizedField(field.trim());
        ...
        if (isNumeric && numValues != null && !noRanges) {
            ...
            HashMap<Double, SingleValueCount> map = new HashMap<>();
            for (IItemId item : ipedResult.getIterator()) {
                int doc = source.getLuceneId(item);
                boolean adv = numValuesSet.advanceExact(doc);
                for (int i = 0; adv && i < numValuesSet.docValueCount(); i++) {
            ...
            ArrayList<SingleValueCount> l = new ArrayList<>(map.values());
            Collections.sort(l);
    ...
    private static class LookupOrdSDV extends LookupOrd {
    ...
    private static class LookupOrdSSDV extends LookupOrd {

        private final SortedSetDocValues ssdv;
