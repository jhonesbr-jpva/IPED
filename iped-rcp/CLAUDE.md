...
- **Quatro parts de topo, um viewer cada** (FR-011) — restauram a estrutura do `ViewerController` legado, onde Hex/Texto/Metadados/Pré-visualização eram **dockables independentes** (não sub-abas de um viewer único). Cada part tem seu **próprio `SwtAwtBridgeHost`** (decisão provisória: isola cada viewer para a futura migração bridge→SWT nativo, part a part):
  ...
  - `part/TextViewerPart` — `RcpTextViewer` (texto extraído, App-free) com **realce** dos termos da busca (amarelo/laranja) + navegação prev/next (`< >`).
...
- **`part/AbstractBridgedViewerPart`** — base comum: o bridge, a assinatura de seleção (`SELECTION_KEY`), e o **render lazy gated por visibilidade** (só carrega o viewer da aba visível — disciplina do `ViewerController` legado; visibilidade via `IPartListener`, flag volátil lida fora da UI thread). Replica a seleção pendente quando o part é criado tardiamente (parts e4 são lazy). Injeta o `SearchService` e plumba os **termos de realce** (`getHighlightTerms()` ← `QueryBuilder.getQueryStrings`) para todos os viewers via `loadIntoViewer(item, source, type, terms)`.
- **`part/HitsViewerPart`** (aba **Ocorrências**, `App.Hits` legado — no `_stackBottom`, ao lado de Itens relacionados): Table SWT nativa de hits do texto (nº + trecho), clicável. Extrai/acha matches **independente** da visibilidade do viewer de Texto, mas compartilha `part/TextExtraction` (extração App-free) + `part/TextHitFinder` (match+snippet) com o `RcpTextViewer` → **offsets idênticos**. Navegação cross-part: `EPartService.activate(textPart)` + publica `part/TextHitGoto{offset,nonce}` no app-context → `TextViewerPart` consome e chama `RcpTextViewer.scrollToOffset` (adia se a extração está em curso, correlação por `loadStamp`).
- `part/RcpAttachmentSearcher`, `bridge/UiThreads`.
- ⚠️ Cada part está em [Application.e4xmi](bundles/iped.rcp.app/Application.e4xmi) com label em `LifeCycle.MODEL_LABEL_KEYS`; o `LAYOUT_VERSION` é bumpado a cada part novo (→5 viewers, →6 Ocorrências) para o `workbench.xmi` persistido resetar (ver §11 — o reset da área **default**, boot sem caso, exige resolver a área pela system property `osgi.instance.area.default`, não pela API `Location`). O wrapper `iped.rcp.libs` exporta `org.exbin.deltahex.*` (assinatura de `HexViewerPlus.HexSearcher`).
