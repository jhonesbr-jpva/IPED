...
- [x] T034 [P] Transition (FR-020/FR-021): RCP app is the promoted interactive case-creation entry (menu File ▸ New/Open Case + Manage Profiles already ship; docs updated — root `CLAUDE.md` §6 + `iped-app/CLAUDE.md` §3 + `iped-rcp/CLAUDE.md` §11.1 — present `ui/iped-ui` as the interactive entry and `iped.exe`/Bootstrap CLI as the unchanged headless entry, removal future/out-of-scope per I1). **SC-006 verified**: the only `iped.exe` token in the interactive-creation code is a comment; the wizard launches `<java.exe> -jar iped.jar` (ProcessingLaunchService), no `iped.exe`, no terminal
...
- [x] T036 Ran [quickstart.md](quickstart.md) V1–V5, all ✅ PASS — recorded in [quickstart-results.md](quickstart-results.md). V1 New Case→process→open; V2 wizard validations (6/6 block); V3 Open Case + invalid-folder; V4 profile create/clone/edit/save + built-in Save As + collision; V5 i18n pt_BR/en (menu+wizard+editor). Found+fixed 4 real issues: SecurityManager launch flag (`4b95f3088`), stale-engine release (rebuild), main-menu i18n scope (`691e1c383`), DS Service-Component header stripping (`6ec9ca55a`). Recent Cases stays deferred (T018/T021). Processing-log review: only benign pre-existing engine noise
