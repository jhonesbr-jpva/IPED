...
- [~] T036 Run [quickstart.md](quickstart.md) validation scenarios V1–V5 and record results in [quickstart-results.md](quickstart-results.md). **V1 ✅ PASS** (New Case → process with `forensic` → open in RCP UI; found+fixed the SecurityManager launch bug `4b95f3088`; stale-engine release rebuilt). V3 Open Case ✅; V4 profile clone ✅ (engine loaded `triage-copy`). Remaining: V2, V3 invalid-folder, V4 re-confirm, V5 i18n `-nl`. Processing-log review: only benign pre-existing engine noise (temp DLL delete-on-Windows, carved-JPEG EOF WARNs, splash reflection, optional numpy)
