...
- **OutlookDBX / java-dbx rebuilt for Tika 3.x → RESOLVED (via JitPack)**: the old `net.sf:java-dbx:1.1-p6` (binary, test-scoped) called `TikaInputStream.get(InputStream, TemporaryResources)`, removed in Tika 3.x → `NoSuchMethodError` masked by an NPE in `OEReader.close`. SEPINF rebuilt the source at [github.com/sepinf-inc/java-dbx](https://github.com/sepinf-inc/java-dbx) tag **`1.1-p7`** (tika-core 3.3.1, 3-arg `get(input, tmp, new Metadata())` — empty metadata, parity-faithful). **Resolved via JitPack** (no iped-maven GitLab publish needed): switched both `net.sf:java-dbx` references to **`com.github.sepinf-inc:java-dbx:1.1-p7`** — `iped-parsers-impl/pom.xml` (test dep) and `iped-app/pom.xml` (`copy-java-dbx` → `plugins/`); `java.dbx.version` = `1.1-p7`. `OutlookDBXParserTest` re-enabled — **passes (28 subitems)**; `iped-app` package copies the jar to `plugins/`. ⚠️ JitPack serves it only under the `com.github.sepinf-inc` groupId; mutating the `1.1-p7` tag again requires purging the `.m2` cache (`com/github/sepinf-inc/java-dbx`).
