...
- **OutlookDBX / java-dbx rebuilt for Tika 3.x → RESOLVED**: `net.sf:java-dbx:1.1-p6` (binary, test-scoped) called `TikaInputStream.get(InputStream, TemporaryResources)`, removed in Tika 3.x → `NoSuchMethodError` masked by an NPE in `OEReader.close`. SEPINF published the source + tag **`1.1-p7`** at [github.com/sepinf-inc/java-dbx](https://github.com/sepinf-inc/java-dbx) (tika-core 3.3.1, 3-arg `get(input, tmp, metadata)`). Bumped `pom.xml` `java.dbx.version` → **1.1-p7**, re-enabled `OutlookDBXParserTest` (removed the soft-skip) — **passes (28 subitems)**. ⚠️ **The 1.1-p7 binary is not yet published to the iped-maven GitLab repo** (resolves only because it was `mvn install`ed locally); publish it (and bump the production java-dbx plugin) or CI/other machines will fail to resolve `net.sf:java-dbx:1.1-p7`.

**Forward-pointers**: the `can_print_degraded`→`can_print_faithful` rename is a **T017** field-stability case; the charset (U+FFFD) and PDFBox-3 glyph (U+FFFD) deltas are **US1 parity** items to weigh in T021. **Publish java-dbx 1.1-p7 binary** to iped-maven GitLab (follow-up).

> **Env note**: the shared local `.m2` is also used by sibling worktrees on the same `4.4.0-SNAPSHOT` version (e.g. `IPED-java21`); a concurrent JDK-21 build there poisons the iped-* SNAPSHOT jars → `UnsupportedClassVersionError (65.0 vs 55.0)` on JDK-11 runs. Build the reactor with `-am` (or full `mvn clean install`) on JDK 11 to refresh `.m2`.
