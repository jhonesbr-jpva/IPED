...
- [X] T104 [US1] `iped_export_item`: exporta um item como arquivo na raiz de escrita configurada, com `text_only` escolhendo entre os bytes do item e o texto extraído (FR-087) — **concluída em 2026-09-05**, com `unit/EvidenceFileNameTest` 9 de 9 e `integration/ItemExportTest` 3 de 3 na bancada, nenhum pulado. Novos: `export/EvidenceFileName` (saneia nome vindo da evidência) e `export/ItemFileWriter` (grava em `.part` e move, com MD5 e SHA-256 calculados no mesmo fluxo que vai para o disco). O texto usa o **mesmo** `ContentAccess.extractText` do `iped_item_text`, com `NO_LIMIT` novo — teto de leitura protege a conversa, não o arquivo. Medido: PDF de 30.218.458 bytes exportado inteiro, `hash_verified` contra o `sha-256` do caso; o mesmo item com `text_only` sai como `.txt` de 5.178 caracteres pelo `PDFParser`; e o item 8318497 (registro decodificado) sai com `source_note` dizendo que o escrito é o preview, não arquivo que existiu na evidência. **A ferramenta não declara `returnsContent`**, e isso é deliberado: a declaração bloqueia a ferramenta inteira, e `binary` recusaria o `text_only` na instalação que libera texto e retém arquivo — a política é aplicada por chamada. O seletor do teste também precisou de correção: `size:[1024 TO 5000000]` **não restringe** o resultado, então a primeira versão pulava conforme o que caísse na página
