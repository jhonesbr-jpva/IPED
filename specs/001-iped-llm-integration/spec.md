...
### Session 2026-09-05 — exportar um item como arquivo

Pedido depois de constatar que o servidor exportava **listas**, não arquivos: o `iped_export_artifact` escreve planilha de metadados, e bytes só saíam item a item pelo `iped_item_content`, em base64 e sob teto de 256 KB.

- Q: O agente escolhe onde salvar? → A: **Não.** Em toda outra escrita o agente nomeia o arquivo; aqui não, porque o nome vem do item e nome de material apreendido é a entrada em que menos se confia. O servidor saneia e decide o caminho, dentro da raiz declarada. O agente recebe o caminho para dizer ao perito onde está. **FR-087 criado.**
- Q: O que o saneamento tira? → A: **Tudo que faz o nome significar caminho.** Separador (`..\..\autoexec.bat`), o dois-pontos de fluxo alternativo (`laudo.txt:oculto`, que grava dentro de um arquivo que *está* na pasta permitida, então confinamento sozinho não pega), caracteres de controle, e ponto ou espaço final — que o Windows descarta em silêncio, fazendo o caminho no resultado apontar para arquivo que não existe. O prefixo com o id neutraliza de uma vez os nomes de dispositivo do Windows: `CON` é dispositivo, `4711-CON` é arquivo. E é ele que liga o arquivo ao item.
- Q: Bytes ou texto? → A: **Booleano, e muda três coisas.** Muda a classe de egresso aplicada (`binary` ou `text`), muda o que se pode conferir — o hash do caso é dos bytes, então o `.txt` não é comparável e o resultado diz isso em vez de deixar um digest com cara de conferência — e muda a extensão. A extração de texto é **a mesma** do `iped_item_text`: duas que pudessem divergir fariam o perito citar algo que o arquivo não diz.
- Q: E os tetos? → A: **Não se aplicam.** Os tetos de `iped_item_content` e `iped_item_text` protegem a conversa. Um PDF de 30 MB sai inteiro, e um texto não para no teto da janela de chat — laudo com o fim faltando não é laudo.
- Q: Por que a ferramenta não declara classe de conteúdo? → A: **Porque não tem uma.** `returnsContent` bloqueia a ferramenta inteira quando a classe não é permitida; declarar `binary` recusaria a exportação de texto justo na instalação que libera texto e retém arquivo, que é a configuração mais provável de querer isso. A política passa a ser aplicada por chamada.
...
- **FR-087**: O sistema MUST permitir exportar **um item identificado** como arquivo, na pasta de escrita declarada na configuração, e MUST permitir escolher entre os **bytes do próprio item** e o **texto extraído** dele. O destino MUST NOT ser parâmetro: o nome do arquivo deriva do item, e nome vindo de material apreendido é entrada não confiável — MUST ser saneado de tudo que o faça significar caminho (separador, fluxo alternativo, ponto ou espaço final que a plataforma descarte em silêncio) e MUST carregar o identificador do item, sem o qual o arquivo não se liga à evidência de onde veio. A exportação de bytes MUST ser conferida contra o hash que o caso registrou, e o resultado MUST declarar o desfecho da conferência — inclusive quando não houver hash para comparar. A exportação de texto MUST usar a mesma extração da leitura de texto: duas extrações que possam divergir fariam o perito citar em laudo algo que o arquivo não diz. Nenhuma das duas MUST ser truncada pelos tetos de leitura, que existem para proteger a conversa e não o arquivo. Item sem conteúdo próprio MUST ser declarado sem escrever nada: arquivo vazio com nome de item é, depois, indistinguível de item vazio.
